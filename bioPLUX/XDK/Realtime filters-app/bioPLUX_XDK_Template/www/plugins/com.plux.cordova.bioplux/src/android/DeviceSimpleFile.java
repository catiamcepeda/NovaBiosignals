...
public class DeviceSimpleFile extends Device {
	...
	String mHeader=""; 
	public DeviceSimpleFile(String port) throws BPException {
		mFileName=port;
		File file;
		if (getStorageState()) {
	...
	public void GetFrames(int nframes, Frame[] frames) throws BPException {
		// File has only some data, no seq and no digital
		try {
			for (int i=0;i<nframes;i++) {
				String line = mReader.readLine();
				String values[] = line.split("\\t");
				frames[i].seq=0;
				frames[i].dig_in =false;
				for (int c=0;c<values.length;c++) {
					frames[i].an_in[c] = (short)Integer.parseInt(values[c]);
				}
			}
		} catch (IOException e) {
